...
camera.framerate = 30
camera.hflip = True  # Horizontal flip langsung dari kamera (seperti cv2.flip(img, 1))
camera.vflip = False  # Set True jika perlu vertical flip

# Inisialisasi array untuk menangkap frame
rawCapture = PiRGBArray(camera, size=(ws, hs))

# Beri waktu untuk kamera stabil
time.sleep(2)

port = "/dev/ttyACM0"  # Port untuk Raspberry Pi, biasanya "/dev/ttyACM0" atau "/dev/ttyUSB0"
...
try:
    # Capture frames from the camera menggunakan generator
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        # Get the array representing the image
        img = frame.array
        
        # Tidak perlu flip manual karena sudah diatur di camera.hflip = True
        # Jika masih perlu adjustment, uncomment salah satu baris berikut:
        ...
        if bboxs:
            ...
            # Dengan hflip=True, mapping normal sudah benar
